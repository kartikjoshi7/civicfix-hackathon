...
import { FiCamera, FiUpload, FiMapPin, FiX, FiRefreshCw, FiActivity, FiCheckCircle, FiAlertTriangle } from 'react-icons/fi';
...
const UserApp = () => {
  ...
  // --- 1. DATA SYNC (Keeps User Stats Fresh) ---
  ...
  useEffect(() => {
    ...
    fetchFreshUserData(); // Initial background sync
  ...
  // --- 3. LOCATION LOGIC (Instant Header Updates) ---
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:9090/analyze-image', { method: 'POST', body: fd });
      const data = await res.json();
      
      if (data.status === 'success' || data.data) {
        const analysis = { ...data.data, location: location || { lat: 0, lng: 0 } };
        setResult(analysis);
      ...
      alert("AI Server Error. Is the backend running?"); 
    } finally { 
      setLoading(false); 
    }
  };

  // --- 5. SAVE REPORT (Instant Count Update) ---
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 font-sans text-gray-800">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="flex items-center gap-1 bg-white px-3 py-1 rounded-full text-xs font-bold text-blue-600 border border-blue-100 shadow-sm">
                <FiMapPin /> {userDetails?.city || 'Location not set'}
              </span>
            </div>
          </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden p-6 md:p-8">
        ...
        {useCamera ? (
          <div className="relative rounded-2xl overflow-hidden bg-black aspect-video mb-6 shadow-inner">
            <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover" />
            <button onClick={capturePhoto} className="absolute bottom-4 left-1/2 -translate-x-1/2 w-16 h-16 border-4 border-white rounded-full bg-red-500 hover:scale-105 transition-transform"></button>
            <button onClick={() => setUseCamera(false)} className="absolute top-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-black/70"><FiX /></button>
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-md group mb-6 bg-gray-50 border border-gray-200">
            ...
            <button onClick={reset} className="absolute top-3 right-3 bg-white text-gray-700 p-2 rounded-full shadow-lg hover:text-red-500 transition-colors"><FiX /></button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <button 
              onClick={() => fileInputRef.current?.click()} 
              className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-blue-200 bg-blue-50/50 rounded-2xl hover:bg-blue-50 hover:border-blue-400 transition-all group"
            >
              <div className="bg-blue-100 text-blue-600 p-3 rounded-full mb-3 group-hover:scale-110 transition-transform"><FiUpload size={24} /></div>
              <span className="font-semibold text-blue-900">Upload Photo</span>
              <span className="text-xs text-blue-400 mt-1">From Gallery</span>
            </button>

            <button 
              onClick={() => setUseCamera(true)} 
              className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-purple-200 bg-purple-50/50 rounded-2xl hover:bg-purple-50 hover:border-purple-400 transition-all group"
            >
              <div className="bg-purple-100 text-purple-600 p-3 rounded-full mb-3 group-hover:scale-110 transition-transform"><FiCamera size={24} /></div>
              <span className="font-semibold text-purple-900">Use Camera</span>
              <span className="text-xs text-purple-400 mt-1">Take Picture</span>
            </button>
        ...
        {/* 2. LOCATION BAR */}
        <div className="bg-gray-50 rounded-xl p-4 flex items-center justify-between border border-gray-100 mb-6 transition-colors hover:border-gray-200">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className={`p-2 rounded-lg ${location ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-400'}`}>
              <FiMapPin />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Location</p>
              <p className="text-sm font-medium text-gray-700 truncate">
                {location?.address || "Location required for report"}
              </p>
            </div>
          </div>
          <button 
            onClick={handleGetLocation} 
            className="text-xs font-bold text-white bg-gray-900 px-4 py-2 rounded-lg ml-2 hover:bg-black transition-colors"
          >
            {location ? 'UPDATE' : 'DETECT'}
          </button>
        </div>

        {/* 3. LOADING / RESULT / ANALYZE BUTTON */}
        {loading ? (
          <div className="bg-blue-50 rounded-2xl p-8 text-center border border-blue-100 animate-pulse">
            <div className="inline-block animate-spin rounded-full h-8 w-8 border-4 border-blue-500 border-t-transparent mb-3"></div>
            <p className="text-blue-800 font-bold">Analyzing Infrastructure...</p>
            <p className="text-xs text-blue-500 mt-1">Powered by Gemini 2.0 Flash</p>
          </div>
        ) : result ? (
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                  result.severity_score >= 7 ? 'bg-red-50 text-red-600 border-red-100' : 
                  result.severity_score >= 4 ? 'bg-yellow-50 text-yellow-600 border-yellow-100' : 
              ...
              <div className="bg-blue-50 p-3 rounded-xl border border-blue-100">
                <p className="text-[10px] text-blue-400 uppercase font-bold mb-1 flex items-center gap-1">
                  <FiAlertTriangle size={10} /> Recommended Action
                </p>
              ...
              <button 
                onClick={reset} 
                className="w-full py-3 bg-gray-900 text-white rounded-xl font-bold hover:bg-black transition-transform active:scale-95"
              >
          ...
          <button 
            ...
            `}
          >
            {image ? 'Analyze & Submit Report' : 'Select Photo to Start'}
          </button>
        )}

        <div className="text-center mt-6">
          <p className="text-[10px] text-gray-400 uppercase tracking-widest font-medium">
            Secure Submission • AI Powered
          </p>
        </div>
