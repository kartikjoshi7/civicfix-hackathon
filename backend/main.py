...
Required JSON Structure:
{
    ...
    "type": "Pothole" | "Garbage Dump" | "Streetlight Failure" | "Electric Pole Damage" | "Waterlogging" | "Broken Pipe/Leakage" | "Manhole Cover Missing" | "Encroachment" | "Traffic Signal Failure" | "Fallen Tree" | "Suspicious/Fake" | "None",
    "severity_score": integer, // 1 (Minor) to 10 (Critical)
    "danger_reason": "string", // Explanation of risk.
    "recommended_action": "string" // Government instruction.
}
...
async def analyze_image(
    ...
    if client:
        try:
            ...
            ai_result = {"issue_detected": False, "error": str(e)}
    else:
...
async def get_all_reports(limit: int = 50):
    ...
    docs = db.collection("reports").order_by("created_at", direction=firestore.Query.DESCENDING).limit(limit).stream()
    return {"reports": [{**doc.to_dict(), "id": doc.id} for doc in docs]}
